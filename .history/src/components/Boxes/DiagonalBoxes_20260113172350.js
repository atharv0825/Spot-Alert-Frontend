...
const TRAVEL = END - START;// full animation path

export default function DiagonalConveyor() {
   const laneA = useRef(new Animated.Value(START)).current;
  const laneB = useRef(new Animated.Value(START + TRAVEL / 2)).current // ↑ This offset prevents both boxes from entering together
